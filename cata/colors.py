...
def initialize_colors(cf):
    ...
    curses.init_pair(
        Color.WEEKEND_NAMES.value, cf.COLOR_WEEKEND_NAMES, cf.COLOR_BACKGROUND
    )
    ...
    curses.init_pair(
        Color.CONFIRMATIONS.value, cf.COLOR_CONFIRMATIONS, cf.COLOR_BACKGROUND
    )
    ...
    curses.init_pair(
        Color.TIMER_PAUSED.value, cf.COLOR_TIMER_PAUSED, cf.COLOR_BACKGROUND
    )
    ...
    curses.init_pair(
        Color.CALENDAR_HEADER.value, cf.COLOR_CALENDAR_HEADER, cf.COLOR_BACKGROUND
    )
    ...
    curses.init_pair(
        Color.CALENDAR_BORDER.value, cf.COLOR_CALENDAR_BORDER, cf.COLOR_BACKGROUND
    )
    ...
    for index in range(len(cf.ICS_EVENT_FILES)):
        if index < len(cf.COLOR_ICS_CALENDARS):
            ...
            color = cf.COLOR_ICS_CALENDARS[
                -1
            ]  # Remaining resources assume the last assigned color
