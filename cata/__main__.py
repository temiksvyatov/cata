...
from cata.calendars import Calendar
from cata.colors import Color, initialize_colors
from cata.configuration import Config
from cata.controls import *
from cata.data import *
from cata.dialogues import clear_line
from cata.errors import Error
from cata.importers import Importer
from cata.loaders import *
from cata.savers import EventSaverCSV, TaskSaverCSV
from cata.screen import Screen
from cata.weather import Weather
...
def read_items_from_user_arguments(
    ...
    try:
        opts, _ = getopt.getopt(
            sys.argv[1:], "pjhvi", ["folder=", "config=", "task=", "event="]
        )
        for opt, arg in opts:
            if opt in "--task":
                name = arg
                user_tasks.add_item(
                    Task(len(user_tasks.items), name, Status.NORMAL, Timer([]), False)
                )
            ...
            if opt in "--event":
                ...
                name = arg.split("-")[3]
                event_id = (
                    user_events.items[-1].item_id + 1
                    if not user_events.is_empty()
                    else 1
                )
                user_events.add_item(
                    UserEvent(
                        event_id,
                        year,
                        month,
...
class View:
    ...
    def fill_background(self):
        ...
        for index in range(y_max - 1):
            self.stdscr.addstr(
                index, 0, " " * x_max, curses.color_pair(Color.EMPTY.value)
            )
        ...
        try:
            if bold and underlined:
                self.stdscr.addstr(
                    ...
                    x,
                    text,
                    curses.color_pair(color.value) | curses.A_BOLD | curses.A_UNDERLINE,
                )
            elif bold and not underlined:
                self.stdscr.addstr(
                    y, x, text, curses.color_pair(color.value) | curses.A_BOLD
                )
            elif underlined and not bold:
                self.stdscr.addstr(
                    y, x, text, curses.color_pair(color.value) | curses.A_UNDERLINE
                )
            else:
                self.stdscr.addstr(y, x, text, curses.color_pair(color.value))
        except (
            curses.error
        ):  # Fix for occasional error with large zoom (reason is unclear)
            return
...
class TaskView(View):
    ...
    def render(self):
        ...
        deadline_view = TaskDeadlineView(
            self.stdscr, self.y, deadline_indentation, self.task
        )
        deadline_view.render()

        addition_indentation = (deadline_view.has_deadline) * (
            4 + len(deadline_view.info)
        )
...
class TimerView(View):
    ...
    def render(self):
        ...
        if self.timer.is_started:
            self.display_line(
                self.y, self.x, f"{self.icon} {self.timer.passed_time}", self.color
            )
...
class JournalView(View):
    ...
    def render(self):
        ...
        if (
            not self.user_tasks.items
            and not self.user_ics_tasks.items
            and cf.SHOW_NOTHING_PLANNED
        ):
...
class EventView(View):
    ...
    def color(self):
        ...
        if self.event.calendar_number is None:
            ...
            for color in Color:
                if (
                    color.value
                    == Color.ICS_CALENDARS0.value + self.event.calendar_number
                ):
                    return color
            return Color.EVENTS
    ...
    def cut_info(self):
        ...
        if (
            cf.CUT_TITLES or cf.SHOW_CALENDAR_BORDERS
        ) and self.screen.calendar_state == CalState.MONTHLY:
...
class DailyView(View):
    ...
    def __init__(
        ...
        self.user_ics_events.items = sorted(
            self.user_ics_events.items, key=lambda x: (x.hour is None, x.hour)
        )
        ...
        self.hidden_events_sign = cf.HIDDEN_ICON + " " * (
            self.screen.x_max - self.x - len(cf.HIDDEN_ICON)
        )
    ...
    def render(self):
        ...
        for event in self.user_events.items:
            if (
                ...
                self.display_line(
                    self.y + self.y_cell - 2,
                    self.x,
                    self.hidden_events_sign,
                    Color.EVENTS,
                )
            else:
                user_event_view = UserEventView(
                    self.stdscr, self.y + index, self.x, event, self.screen
                )
                user_event_view.render()

                if (
                    self.screen.selection_mode
                    and self.is_selection_day
                    and self.screen.state == AppState.CALENDAR
                ):
                    self.display_line(
                        self.y + index,
                        self.x,
                        str(index + self.index_offset + 1),
                        Color.ACTIVE_PANE,
                    )
            index += 1
        ...
        for event_list in [
            self.repeated_user_events.items,
            self.user_ics_events.items,
            self.repeated_ics_events.items,
        ]:
...
class DayNumberView(View):
    ...
    def render(self):
        ...
        if self.screen.date == self.screen.today:
            today = (
                f"{self.day}{cf.TODAY_ICON}{' '*(self.x_cell - len(str(self.day)) - 2)}"
            )
            self.display_line(
                self.y, self.x, today, Color.TODAY, cf.BOLD_TODAY, cf.UNDERLINED_TODAY
            )
            ...
            self.display_line(
                self.y, self.x, weekday, Color.DAYS, cf.BOLD_DAYS, cf.UNDERLINED_DAYS
            )
...
class HeaderView(View):
    ...
    def render(self):
        ...
        title_view = TitleView(
            self.stdscr, 0, self.screen.x_min, self.title, self.screen
        )
        title_view.render()
        ...
        if cf.SHOW_WEATHER and size_allows:
            self.display_line(
                0,
                self.screen.x_max - len(self.weather.forcast) - 1,
        ...
        size_allows = len(self.weather.forcast) < self.screen.x_max - len(
            self.title
        ) - len(time_string)
...
class SeparatorView(View):
    ...
    def render(self):
        ...
        height = (
            self.screen.number_of_weeks * y_cell + 2
            if cf.SHOW_CALENDAR_BORDERS
            else self.screen.y_max
        )
        ...
        if cf.SHOW_CALENDAR_BORDERS and self.screen.calendar_state == CalState.MONTHLY:
            for row in range(1, self.screen.number_of_weeks):
                self.display_line(
                    row * y_cell + 1, x_separator, "┤", Color.CALENDAR_BORDER
                )
            self.display_line(
                self.screen.number_of_weeks * y_cell + 1,
                x_separator,
                "┘",
    ...
    def render(self):
        ...
        for column in range(1, 7):
            for row in range(y_cell * self.rows):
                self.display_line(
                    row + 2, column * x_cell - 1, "│", Color.CALENDAR_BORDER
                )
        ...
        for row in range(1, self.rows):
            for column in range(1, 7):
                self.display_line(
                    row * y_cell + 1, column * x_cell - 1, "┼", Color.CALENDAR_BORDER
                )
        for column in range(1, 7):
            self.display_line(
                self.rows * y_cell + 1, column * x_cell - 1, "┴", Color.CALENDAR_BORDER
            )
...
class DailyScreenView(View):
    ...
    def dates(self):
        ...
        return Calendar(
            cf.START_WEEK_DAY - 1, cf.USE_PERSIAN_CALENDAR
        ).monthdayscalendar(self.screen.year, self.screen.month)
    ...
    def render(self):
        ...
        header_view = HeaderView(
            self.stdscr, 0, 0, date_string, self.weather, self.screen
        )
        header_view.render()
        ...
        repeated_ics_events = RepeatedEvents(
            self.user_ics_events, cf.USE_PERSIAN_CALENDAR
        )
        ...
        for _ in range(max_num_days):
            ...
            day_string += " " * (
                max([len(day_name) for day_name in DAYS])
                + 3
                + len(self.icon)
                - len(day_string)
            )
            self.display_line(
                self.y + 2 + vertical_shift, self.x, day_string, self.color
            )

            # Display events of the day:
            daily_view = DailyView(
                self.stdscr,
                self.y + 3 + vertical_shift,
                self.x,
                repeated_user_events,
                repeated_ics_events,
                self.user_events,
...
class MonthlyScreenView(View):
    ...
    def render(self):
        ...
        month_names = MONTHS
        month_year_string = (
            month_names[self.screen.month - 1] + " " + str(self.screen.year)
        )
        dates = Calendar(
            cf.START_WEEK_DAY - 1, cf.USE_PERSIAN_CALENDAR
        ).monthdayscalendar(self.screen.year, self.screen.month)
        ...
        header_view = HeaderView(
            self.stdscr, 0, 0, month_year_string, self.weather, self.screen
        )
            ...
            for col, day in enumerate(week):
                if day != 0:
                    ...
                    day_in_week = (
                        col
                        + (cf.START_WEEK_DAY - 1)
                        - 7 * ((col + (cf.START_WEEK_DAY - 1)) > 6)
                    )
                    day_number_view = DayNumberView(
                        self.stdscr,
                        2 + row * y_cell,
                        col * x_cell,
                        self.screen,
                    ...
                    daily_view = DailyView(
                        ...
                        self.user_ics_tasks,
                        self.screen,
                        num_events_this_month,
                    )
                    daily_view.render()
                    num_events_this_month += len(
                        self.user_events.filter_events_that_day(self.screen).items
                    )
...
class JournalScreenView(View):
    ...
    def render(self):
        ...
        # Display header and footer:
        header_view = HeaderView(
            self.stdscr, 0, 0, cf.JOURNAL_HEADER, self.weather, self.screen
        )
        header_view.render()
...
class HelpScreenView(View):
    ...
    def render(self):
        ...
        self.display_line(
            self.global_shift_y,
            self.global_shift_x + 1,
            f"{MSG_NAME} {__version__}",
            Color.ACTIVE_PANE,
            cf.BOLD_TITLE,
            cf.UNDERLINED_TITLE,
        )
        self.display_line(
            self.global_shift_y + 2,
            self.global_shift_x + 8,
            TITLE_KEYS_GENERAL,
            Color.TITLE,
            cf.BOLD_TITLE,
            cf.UNDERLINED_TITLE,
        ...
        self.display_line(
            self.global_shift_y + 4 + len(KEYS_GENERAL),
            self.global_shift_x + 8,
            TITLE_KEYS_CALENDAR,
            Color.TITLE,
            cf.BOLD_TITLE,
        ...
        for index, key in enumerate(KEYS_CALENDAR):
            self.display_line(
                self.global_shift_y + index + 5 + len(KEYS_GENERAL),
                self.global_shift_x,
                key,
                Color.ACTIVE_PANE,
            )
...
def main(stdscr) -> None:
    ...
    read_items_from_user_arguments(
        screen, user_tasks, user_events, task_saver_csv, event_saver_csv
    )
    ...
    monthly_screen_view = MonthlyScreenView(
        stdscr,
        0,
        0,
        weather,
    ...
    daily_screen_view = DailyScreenView(
        stdscr,
        0,
        0,
        weather,
    ...
    journal_screen_view = JournalScreenView(
        stdscr, 0, 0, weather, user_tasks, user_ics_tasks, screen
    )
