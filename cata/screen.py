...
from cata.data import Events, AppState, CalState
...
class Screen:
    ...
    def __init__(self, stdscr, cf):
        ...
        self.calendar_state = (
            CalState.DAILY if cf.DEFAULT_CALENDAR_VIEW == "daily" else CalState.MONTHLY
        )
    ...
    def number_of_weeks(self) -> int:
        ...
        return len(
            Calendar(0, self.use_persian_calendar).monthdayscalendar(
    ...
    def next_day(self):
        ...
        days_in_this_month = Calendar(0, self.use_persian_calendar).last_day(
            self.year, self.month
        )
    ...
    def previous_day(self):
        ...
        if self.day > 1:
            ...
            if self.month > 1:
                ...
                self.month = 12
                self.year -= 1
            self.day = Calendar(0, self.use_persian_calendar).last_day(
                self.year, self.month
            )
    ...
    def is_valid_day(self, number) -> bool:
        ...
        return (
            0
            < number
            <= Calendar(0, self.use_persian_calendar).last_day(self.year, self.month)
        )
    ...
    def is_valid_date(self, year, month, day) -> bool:
        ...
        if 1 <= month <= 12:
            return (
                0 < day <= Calendar(0, self.use_persian_calendar).last_day(year, month)
            )
        return False
