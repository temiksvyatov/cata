...
class UserRepeatedEvent(Event):
    ...
    def __init__(
        self, item_id, year, month, day, name, status, privacy, calendar_number=None
    ):
        super().__init__(year, month, day, name)
        self.item_id = item_id
        self.status = status
...
class Collection:
    ...
    def filter_events_that_month(self, screen):
        ...
        events_of_the_month.items = sorted(
            events_of_the_month.items, key=lambda item: item.day
        )
        ...
        for event in self.user_events.items:
            if event.repetition >= 1:
                for rep in range(1, event.repetition):
                    ...
                    temp_month = event.month + rep * (
                        event.frequency == Frequency.MONTHLY
                    )
                    temp_day = (
                        event.day
                        + rep * (event.frequency == Frequency.DAILY)
                        + 7 * rep * (event.frequency == Frequency.WEEKLY)
                    )
                    year, month, day = self.calculate_recurring_events(
                        temp_year, temp_month, temp_day, event.frequency
                    )
                    self.add_item(
                        UserRepeatedEvent(
                            event.item_id,
    ...
    def calculate_recurring_events(self, year, month, day, frequency):
        ...
        if frequency in [Frequency.WEEKLY, Frequency.DAILY]:
            ...
            for i in range(1000):
                ...
                last_day = Calendar(0, self.use_persian_calendar).last_day(
                    year, month + i
                )
