...
class LoaderCSV:
    ...
    def create_file(self, filename):
        ...
        try:
            ...
            logging.error(
                "Problem occured trying to create %s. %s", filename, e_message
            )
            return []
...
class HolidayLoader:
    ...
    def load_country(self, country):
        ...
        try:
            ...
            holiday_events = (getattr(hl, country))(
                years=[year + x for x in range(-2, 5)]
            )
            for date, name in holiday_events.items():
                # Convert to persian date if needed:
                if self.use_persian_calendar:
                    year, month, day = convert_to_persian_date(
                ...
                holiday = Event(
                    ...
                    month,
                    day,
                    f"{name} ({country_code})" if len(self.countries) > 1 else name,
                )
                self.holidays.add_item(holiday)

        except ModuleNotFoundError:
            logging.error(
                "Couldn't load holidays. Module holidays is not installed. Try 'pip install holidays'"
            )
            pass
        except (SyntaxError, AttributeError) as e_message:
            logging.error(
                "Couldn't load holidays. Country might be incorrect. %s", e_message
            )
            pass
        return self.holidays
...
class BirthdayLoader:
    ...
    def load(self):
        ...
        if not self.abook_file.exists():
            logging.warning(
                "Couldn't load birthdays. File. %s does not exist.",
                str(self.abook_file),
            )
            return self.birthdays
...
class LoaderICS:
    ...
    def read_url(self, path):
        ...
        try:
            ...
            logging.error(
                "Failed to load from %s. Probably no internet connection. %s",
                path,
                e_message,
            )
            return ""
...
class EventLoaderICS(LoaderICS):
    ...
    def parse_event(self, component, index, calendar_number):
        ...
        all_day = (
            component.get("dtstart").params.get("VALUE") == "DATE"
