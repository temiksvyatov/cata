...
def control_monthly_screen(stdscr, screen, user_events, importer):
    ...
    if screen.selection_mode:
        ...
        if screen.key in ["i", "h"]:
            ...
            if user_events.filter_events_that_month(screen).is_valid_number(number):
                event_id = (
                    user_events.filter_events_that_month(screen).items[number].item_id
                )
        ...
        if screen.key in ["e", "r"]:
            ...
            if user_events.filter_events_that_month(screen).is_valid_number(number):
                ...
                new_name = input_string(
                    ...
                    0,
                    MSG_NEW_TITLE,
                    screen.x_max - len(MSG_NEW_TITLE) - 2,
                )
            ...
            if user_events.filter_events_that_month(screen).is_valid_number(number):
                ...
                if screen.key == "m":
                    year, month, day = input_date(
                        stdscr, screen.y_max - 2, 0, MSG_EVENT_MV_TO
                    )
    ...
    # Otherwise, we check for user input:
        ...
        if (
            screen.key in selection_keys
            and user_events.filter_events_that_month(screen).items
        ):
        ...
        if screen.key == "a":
            ...
            if screen.is_valid_day(day):
                ...
                name = input_string(
                    ...
                    0,
                    MSG_EVENT_TITLE,
                    screen.x_max - len(MSG_EVENT_TITLE) - 2,
                )
                event_id = (
...
def control_daily_screen(stdscr, screen, user_events, importer):
    ...
    if screen.selection_mode:
        ...
        if screen.key in ["i", "h"]:
            ...
            if user_events.filter_events_that_day(screen).is_valid_number(number):
                item_id = (
                    user_events.filter_events_that_day(screen).items[number].item_id
                )
    ...
    # Otherwise, we check for user input:
        ...
        if (
            screen.key in selection_keys
            and user_events.filter_events_that_day(screen).items
        ):
            screen.selection_mode = True
        ...
        if screen.key == "a":
            ...
            item_id = (
                user_events.items[-1].item_id + 1 if not user_events.is_empty() else 1
            )
            user_events.add_item(
                UserEvent(
                    item_id,
...
def control_journal_screen(stdscr, screen, user_tasks, importer):
    ...
    if screen.selection_mode:
        ...
        if screen.key == "f":
            ...
            if user_tasks.is_valid_number(number):
                ...
                clear_line(stdscr, screen.y_max - 2, 0)
                year, month, day = input_date(
                    stdscr, screen.y_max - 2, 0, MSG_TS_DEAD_DATE
                )
                if screen.is_valid_date(year, month, day):
        ...
        if screen.key == "A":
            ...
            if user_tasks.is_valid_number(number):
                clear_line(stdscr, screen.y_max - 2, 0)
                task_name = input_string(
                    stdscr,
                    screen.y_max - 2,
                    0,
                    MSG_TS_TITLE,
                    screen.x_max - len(MSG_TS_TITLE) - 2,
                )
                task_id = user_tasks.generate_id()
                user_tasks.add_subtask(
                    Task(task_id, task_name, Status.NORMAL, Timer([]), False), number
                )
        screen.selection_mode = False
    ...
    # Otherwise, we check for user input:
        ...
        if screen.key == "a":
            ...
            task_id = user_tasks.generate_id()
            user_tasks.add_item(
                Task(task_id, task_name, Status.NORMAL, Timer([]), False)
            )
