...
from cata.calendars import convert_to_gregorian_date
        ...
        with open(dummy_file, "w", encoding="utf-8") as f:
            for task in self.user_tasks.items:
                ...
                if self.use_persian_calendar and task.year != 0:
                    year, month, day = convert_to_gregorian_date(
                ...
                dot = "."
                f.write(
                    f'{year},{month},{day},"{dot*task.privacy}{task.name}",{task.status.name.lower()}'
                )
    ...
    def save(self):
        ...
        with open(dummy_file, "w", encoding="utf-8") as file:
            for ev in self.user_events.items:
                ...
                file.write(
                    f'{ev.item_id},{year},{month},{day},"{name}",{ev.repetition},{ev.frequency.name.lower()},{ev.status.name.lower()}\n'
                )
        dummy_file.replace(original_file)
